...
export default defineConfig(({ mode }) => {
  // Path the built app is served from, e.g. "/karavindasar/ot-confluence-bot/" for a project
  // site on a static host that doesn't own the domain root. Read from the environment — a
  // VITE_BASE_PATH in a .env file, a host's build-time env vars, or a Docker build ARG — rather
  // than hardcoded here, so nothing in the source needs to change when that path does. Left
  // unset, it defaults to '/', which is what the Docker/nginx deployment already assumes.
